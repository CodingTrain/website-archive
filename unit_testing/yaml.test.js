...
const { markdownFilenameToUrl, resolveLocalUrl, urlToRepositoryDirectoryFilename, describeIf, itIf } = require("./helpers");
...
const checkFolder = (videoFormat, name, { directories, files }) =>
  describe(name, () => {
    ...
    for (const [fileName, file] of Object.entries(files)) {
      ...
      describe(fileName, () => {
        ...
        if (valid) {
          const { video_id, web_editor, repository } = file.content;
        ...
        itIf(valid && file.content.video_id)("Has a unique video_id", () => {
          ...
          if (fileList.length !== 1) {
            throw new Error(
              `video_id is shared with: ${fileList
                .filter((f) => f !== file)
                .map((f) => f.path)
                .join(", ")}`,
            );
        ...
        itIf(valid)("has a valid repository to web_editor mapping", () => {
          ...
          const { repository, web_editor } = file.content;
